...
private def checkNotation (e : Expr) (srcStx : Syntax) :
  ...
  -- Skip if delab chose dot notation — the dotNotation linter handles that
  if delabStx.raw.isOfKind ``Parser.Term.proj then return none
  -- Get the suggestion text via ppExpr (clean formatting, no reprint spacing)
  let fmt ← PrettyPrinter.ppExpr e
  let suggestion := fmt.pretty.trimAscii.toString
