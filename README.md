...
For dependent types, use `_` as a wildcard to match any argument:
...
-- Bad: x doesn't start with l or l'
...
with any `notation`-defined syntax (not `macro`-defined).
...
Also catches `HAdd.hAdd a b` (suggest `a + b`), `And p q` (suggest
